...
class FileVersionsListener implements IEventListener {
    ...
    public function versionRestored(IVersion $version) {
        ...
        $filePath = preg_replace('/^\/\w+\/files\//', '', $node->getPath());
        if (empty($filePath)) {
            return;
        }
        ...
        try {
            ...
            FileVersions::deleteVersion($ownerId, $node, $versionId);
        } catch (\Exception $e) {
            $this->logger->error(
                "VersionRestoredEvent: " . json_encode($version),
